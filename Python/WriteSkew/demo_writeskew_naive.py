...
stmt_count = '''SELECT COUNT(*) FROM bank_accounts WHERE is_active = TRUE'''
...
def deactivator(id_, delay):
    ...
    try:
        ...
        conn.close()
t1 = threading.Thread(target=deactivator, args=(1, 2))
t2 = threading.Thread(target=deactivator, args=(2, 0.5))
t1.start(); t2.start()
t1.join(); t2.join()